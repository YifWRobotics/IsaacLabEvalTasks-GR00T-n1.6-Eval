...
class Gr00tN1ClosedLoopArguments:
    ...
    arm_stiffness: float = field(
        default=3000.0,
        metadata={
            "description": "Arm joint stiffness (Kp) applied to both left-arm and right-arm actuators during eval."
        },
    )
    arm_damping: float = field(
        default=100.0,
        metadata={
            "description": "Arm joint damping (Kd) applied to both left-arm and right-arm actuators during eval."
        },
    )
    hand_stiffness: Optional[float] = field(
        default=None,
        metadata={
            "description": "Optional hand joint stiffness (Kp) for left-hand/right-hand actuators. If None, keep env defaults."
        },
    )
    hand_damping: Optional[float] = field(
        default=None,
        metadata={
            "description": "Optional hand joint damping (Kd) for left-hand/right-hand actuators. If None, keep env defaults."
        },
    )
    action_smoothing_alpha: float = field(
        default=0.35,
        metadata={
            "description": "EMA coefficient for actions in [0,1]. 1.0 disables smoothing; lower values smooth more."
        },
    )
    max_action_delta: Optional[float] = field(
        default=0.08,
        metadata={
            "description": "Optional per-joint max delta (rad/step) applied after smoothing. If None, no delta clamp."
        },
    )
    ...
    def __post_init__(self):
        ...
        assert self.arm_stiffness > 0.0, "arm_stiffness must be > 0"
        assert self.arm_damping > 0.0, "arm_damping must be > 0"
        if self.hand_stiffness is not None:
            assert self.hand_stiffness > 0.0, "hand_stiffness must be > 0 when provided"
        if self.hand_damping is not None:
            assert self.hand_damping > 0.0, "hand_damping must be > 0 when provided"
        assert 0.0 < self.action_smoothing_alpha <= 1.0, "action_smoothing_alpha must be in (0, 1]"
        if self.max_action_delta is not None:
            assert self.max_action_delta > 0.0, "max_action_delta must be > 0 when provided"
        # embodiment_tag
